...
short_description: Copy image to Secondary Flash on Brocade Switch
description:
    - Copy image to Secondary Flash on Brocade Switch
...
- name: Brocade ICCX6610 Switch Post Staging
  ...
  tasks:
    - name: Copy TFTP to Secondary
      brocade_copy_tftp__flash_secondary:
         host="{{ inventory_hostname }}"
         softwareImage="{{ softwareImage }}"
         tftpServer="{{ tftpServer }}"
         username="{{ username }}"
...
def brocade_copy_tftp_flash_secondary(module):
    hostname = module.params['host']
    softwareImage = module.params['softwareImage']
    tftpServer = module.params['tftpServer']
    username = module.params['username']
    password = module.params['password']

    # For now the log file is mandatory and always created for debugging
    # and verification.
    # To do list includes making it optional
    # It looks like since Ansible copies the script into a temp folder
    # theabsolute path is required for the directory
    # There is probably a more "ansible" way set a directory but this
    # works ok for now
    ...
    try:
        ...
        child.expect('.*#.*')
        child.sendline('copy tftp flash ' + tftpServer + ' ' +
                       softwareImage + ' secondary')
        child.expect('.*TFTP to Flash Don.*', timeout=300)
        # child.expect('.*#.*')
        ...
        results['failed'] = True
        msg = "EOF error -- unable to connect to {}".format(
            module.params['host'])
        results['msg'] = msg
        logging.info('EOF Error on {}'.format(module.params['host']))
        logging.info(err)
        module.fail_json(msg='ERROR -- Unable to connect to {}'.format(
            module.params['host']))
    except TIMEOUT, err:
        results['failed'] = True
        msg = "TIMEOUT error -- did not get expected values returned " + \
              "on {}".format(module.params['host'])
        results['msg'] = msg
        logging.info('TIMEOUT Error on {}'.format(module.params['host']))
        logging.info(err)
        module.fail_json(
            msg='ERROR - Did not get expected values returned ' +
                'on {}'.format(module.params['host'])
        )
    return results
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            softwareImage=dict(required=True),
    ...
    results = brocade_copy_tftp_flash_secondary(module)
